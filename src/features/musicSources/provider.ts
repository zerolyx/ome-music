 ...
 *
 * Cookie present never implies authenticated. HTTP 403 on Direct QR is
 * "failed / unavailable", never "expired".
 */
export type QQMusicLoginFlowStatus =
  | "idle"
  | "opening_login"
  | "waiting_for_user"
  | "collecting_cookies"
...
export interface NetEaseServiceStatus {
  ...
  /**
   * P1-8: machine-readable reason when the service is not running
   * (`runtime_missing` / `spawn_failed` / `health_timeout` /
   * `external_source`). Lets the UI say what actually went wrong instead of a
   * generic "not ready".
   */
  reason?: string | null;
}
