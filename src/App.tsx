...
import {
  ...
  nextPlayableIndex,
  type PlaybackFailureTracker,
  type StopCode,
} from "./features/playback/playbackFailurePolicy";
import {
  createPlaybackFailureError,
  isConnectableReason,
  playbackNoticeLabel,
  playbackReasonMessage,
  reasonFromPlaybackError,
} from "./features/playback/playbackFailureReason";
...
/**
 * Terminal messages for the P1-7 auto-skip state machine. These are the cases
 * where the queue deliberately does NOT advance, so the reason has to be
 * explicit — a silent stop here is exactly the bug being fixed.
 */
function playbackStopMessage(code: StopCode, reason?: string | null, queueLength = 0): string {
  switch (code) {
    case "repeat_one":
      return "This track can't play, and repeat-one will not skip past it. Turn repeat off to continue.";
    case "queue_exhausted":
      return queueLength > 0
        ? `No playable track in the current queue — ${queueLength} tracks were tried and none could play.`
        : "The queue is empty.";
    case "manual_failure":
    default:
      return playbackReasonMessage(reason);
  }
}
...
export default function App() {
  ...
  // Whether the current track was chosen by the user or reached by the queue
  // advancing on its own. P1-7 uses this to keep the two UX paths different:
  // a manual click reports and stays, an auto-advance skips and continues.
  const playbackOriginRef = useRef<"auto" | "manual">("manual");
  const loopModeRef = useRef<LoopMode>("all");
  // P1-7: single authority for "what do we do after a track fails to play".
  const playbackFailureTrackerRef = useRef<PlaybackFailureTracker>(createPlaybackFailureTracker());
  ...
  // P1-7: low-interference notice shown when the queue auto-skips a dead
  // track. Distinct from `libraryError`, which is the terminal "we stopped"
  // notice. This one is transient and auto-clears so it never competes with
  // the music for attention.
  const [skippedNotice, setSkippedNotice] = useState<{
    message: string;
    reason: string | null;
  } | null>(null);
  const skippedNoticeTimerRef = useRef<number | null>(null);
  ...
  // P1-7: the failure handler is bound once at mount, so the queue needs to be
  // readable from a ref when it decides whether and where to advance.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const resolvePlayable = async () => {
      ...
      if (sourceId) {
        ...
        if (!result.url || result.unavailable) {
          throw createPlaybackFailureError(result.reason);
        }
      ...
      if (!src || isTrackUnavailable(currentTrack)) {
        throw createPlaybackFailureError(currentTrack.unavailableReason);
      }
      return { audioUrl: src };
    };
    ...
    resolvePlayable()
      ...
      .catch((error) => {
        ...
        preparedPlayableRef.current.delete(currentTrack.id);
        audioRef.current?.pause();
        // P1-7: a resolve failure used to be terminal for the whole queue.
        // It now goes through the same retry / skip / stop authority as an
        // audio-element error.
        handlePlaybackFailureRef.current(
          currentTrack,
          reasonFromPlaybackError(error) ?? currentTrack.unavailableReason ?? null,
  ...
  // P1-7 — the single failure authority.
  //
  // Every terminal playback failure (resolve failure, audio element error)
  // funnels through here instead of each site deciding on its own. Before this
  // existed both failure paths only did `setIsPlaying(false)`, so a single
  // unplayable track inside a large restored queue stopped playback forever.
  type PlaybackFailureHandler = (
    track: Track,
    reason: string | null,
    origin: "auto" | "manual",
    retryAllowed: boolean,
  ) => void;
  const handlePlaybackFailureRef = useRef<PlaybackFailureHandler>(() => {});
  type AdjacentTrackHandler = (
    direction: "next" | "previous",
    options?: { markSkip?: boolean; avoidUnavailable?: boolean },
  ) => void;
  const playAdjacentTrackRef = useRef<AdjacentTrackHandler>(() => {});

  useEffect(() => {
    const showSkippedNotice = (track: Track, reason: string | null) => {
      setSkippedNotice({
        message: `Skipped "${track.title}" — ${playbackReasonMessage(reason)}`,
        reason,
      });
      if (skippedNoticeTimerRef.current !== null) {
        window.clearTimeout(skippedNoticeTimerRef.current);
      }
      skippedNoticeTimerRef.current = window.setTimeout(() => setSkippedNotice(null), 4500);
    };

    handlePlaybackFailureRef.current = (track, reason, origin, retryAllowed) => {
      const tracker = playbackFailureTrackerRef.current;
      ...
      const decision = tracker.decide({
        ...
        reason,
        origin,
        repeatOne: loopModeRef.current === "one",
        queueLength: queue.length,
        skippedInPass: tracker.getSkippedInPass(),
        retryAllowed,
      });

      if (decision.kind === "retry") {
        // Keep the legacy counters in sync so the existing resolve guard still
        // lets a fresh URL through.
        playbackRetryKeyRef.current = `${track.id}:${playbackQualityRef.current}:${track.source}`;
        playbackRetryCountRef.current = decision.attempt;
        setLibraryError(null);
        setPlaybackRetryNonce((value) => value + 1);
        return;
      }

      tracker.markUnavailable(track.id);
      setPlaybackRetryNonce(0);

      if (decision.kind === "stop") {
        setIsPlaying(false);
        setLibraryError(playbackStopMessage(decision.code, reason, queue.length));
        return;
      }

      // Skip: advance to the next track that is not already known to be dead.
      const hasCandidate =
        nextPlayableIndex({
          length: queue.length,
          fromIndex: currentIndexRef.current,
          direction: 1,
          unavailable: tracker.unavailableIds(),
          ids: queue.map((item) => item.id),
        }) !== null;

      if (!hasCandidate) {
        setIsPlaying(false);
        setLibraryError(playbackStopMessage("queue_exhausted", reason, queue.length));
        return;
      }

      showSkippedNotice(track, reason);
      playAdjacentTrackRef.current("next", { markSkip: false, avoidUnavailable: true });
    };
  });
  ...
  useEffect(() => {
    ...
    const handlePlaybackReady = () => {
      const track = currentTrackRef.current;
      playbackFailureTrackerRef.current.notePlaybackReady(track?.id ?? "");
      playbackRetryCountRef.current = 0;
    };
    const handleError = () => {
      const track = currentTrackRef.current;
      if (!track || !playableSrcRef.current) return;
      // A cached proxied URL (ome-media:// token) can expire or the remote CDN
      // can refuse the request, so re-resolving may genuinely help. A local
      // file path will never change, so retrying it would only waste time.
      ...
      playableSrcForTrackIdRef.current = null;
      setPlayableSrc("");
      // P1-7: retry / skip / stop is now decided in one place.
      handlePlaybackFailureRef.current(
        track,
        track.unavailableReason ?? null,
        playbackOriginRef.current,
        looksLikeProxiedRemoteUrl,
      );
    };
  ...
  const playAdjacentTrack = (
    ...
    const { markSkip = true, avoidUnavailable = false } = options;
    // Everything the queue starts on its own is an auto-advance; P1-7 treats a
    // failure on it very differently from a failure on a track the user picked.
    playbackOriginRef.current = "auto";
    ...
    if (shuffle) {
      ...
      if (avoidUnavailable) {
        const unavailableIds = playbackFailureTrackerRef.current.unavailableIds();
        let guard = 0;
        while (
          unavailableIds.has(tracks[nextIndex]?.id ?? "") &&
          guard < shuffleOrderRef.current.length
        ) {
          nextIndex = shuffleOrderRef.current[shufflePositionRef.current] ?? currentIndex;
          shufflePositionRef.current += 1;
          guard += 1;
      ...
      const plainNext = (currentIndex + offset + tracks.length) % tracks.length;
      nextIndex = avoidUnavailable
        ? (nextPlayableIndex({
            length: tracks.length,
            fromIndex: currentIndex,
            direction: offset === 1 ? 1 : -1,
            unavailable: playbackFailureTrackerRef.current.unavailableIds(),
  ...
  // The failure handler is bound once at mount, so it reaches the advance
  // logic through a ref that always points at the latest closure.
  useEffect(() => {
  ...
  const togglePlay = () => {
    ...
    playbackRetryCountRef.current = 0;
    setPlaybackRetryNonce(0);
    // P1-7: a user-initiated play must never be yanked away by auto-skip.
    playbackOriginRef.current = "manual";
    playbackFailureTrackerRef.current.noteManualPlay(currentTrack.id);
  ...
  const playLocalTrack = (track: Track) => {
    ...
    // P1-7: the user picked this track, so a failure reports instead of
    // navigating away from it.
    playbackOriginRef.current = "manual";
    playbackFailureTrackerRef.current.noteManualPlay(track.id);
    lastPlayEventKeyRef.current = null;
  ...
  // The skip notice is transient and wins while it lives; `libraryError` is
  // the terminal "playback stopped" notice. Only one is ever on screen, so the
  // player never stacks competing banners.
  const activeNotice =
    skippedNotice ??
    (libraryError
      ? {
          message: libraryError,
          reason: playbackDebug?.reason ?? currentTrack?.unavailableReason ?? null,
        }
      : null);
