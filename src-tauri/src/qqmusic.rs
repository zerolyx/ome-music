...
/// Direct QR 不可用的统一错误文案。
/// 403 / 反自动化页是「环境不可用」，绝不能显示为「二维码已过期」。
pub(crate) const QQ_DIRECT_QR_UNAVAILABLE: &str =
    "direct_qr_unavailable: 当前环境无法使用直接扫码（QQ 服务端拒绝），请使用官方登录 / Direct QR is unavailable on this network; use Official Sign-in.";

/// 把 ptlogin HTTP 状态映射为 Direct QR 可用性错误。
/// 纯函数，便于单元测试；403 ≠ expired。
pub(crate) fn ensure_direct_qr_http_ok(status: reqwest::StatusCode) -> Result<(), String> {
    if status == reqwest::StatusCode::FORBIDDEN
        || status == reqwest::StatusCode::UNAUTHORIZED
        || status == reqwest::StatusCode::PROXY_AUTHENTICATION_REQUIRED
    {
        return Err(QQ_DIRECT_QR_UNAVAILABLE.to_string());
    }
    if status.is_client_error() || status.is_server_error() {
        return Err(format!(
            "direct_qr_unavailable: QQ 登录接口返回 HTTP {}，请使用官方登录 / QQ login endpoint returned HTTP {}; use Official Sign-in.",
            status.as_u16(),
            status.as_u16()
        ));
    }
...
pub async fn create_qqmusic_qr() -> Result<QQMusicQrLoginDto, String> {
    ...
    ensure_direct_qr_http_ok(xlogin_resp.status())?;
    ...
    ensure_direct_qr_http_ok(resp.status())?;
    ...
    if content_type.contains("text/html") || content_type.contains("text/plain") {
        // 200 但返回验证页 / 反自动化页：同样属于 Direct QR 不可用。
        return Err(QQ_DIRECT_QR_UNAVAILABLE.to_string());
    }
    let img_bytes = resp
        .bytes()
...
mod tests {
    ...
    #[test]
    fn direct_qr_403_is_unavailable_not_expired() {
        let err = ensure_direct_qr_http_ok(reqwest::StatusCode::FORBIDDEN).unwrap_err();
        assert!(err.starts_with("direct_qr_unavailable:"));
        assert!(err.contains("官方登录"));
        assert!(!err.to_ascii_lowercase().contains("expired"));
        assert!(!err.to_ascii_lowercase().contains("过期"));

        assert!(ensure_direct_qr_http_ok(reqwest::StatusCode::OK).is_ok());
        let bad = ensure_direct_qr_http_ok(reqwest::StatusCode::TOO_MANY_REQUESTS).unwrap_err();
        assert!(bad.starts_with("direct_qr_unavailable:"));
        assert!(bad.contains("429"));
    }

    #[test]
    fn diagnostic_metadata_lists_cookie_names_without_values() {
        let cookie = "uin=o0012345; qm_keyst=secret==; p_skey=private";
        let names: Vec<String> = parse_cookie_header(cookie)
            .iter()
            .map(|entry| entry.name.clone())
            .collect();
        assert_eq!(names, vec!["uin", "qm_keyst", "p_skey"]);
        assert!(!cookie_name_list(cookie).contains("secret"));
        assert!(!cookie_name_list(cookie).contains("private"));
        assert!(!cookie_name_list(cookie).contains("o0012345"));
    }
}
