...
pub struct AppState {
    ...
    /// P1-8: the single authority for "is the NetEase sidecar actually ready".
    /// `process exists` was never the same thing as `port is listening`, so the
    /// readiness result of the last transition is stored here instead of being
    /// re-derived from the child handle on every request.
    netease_readiness: Arc<Mutex<NeteaseReadiness>>,
}

/// QQ 官方 WebView 登录流程的对外只读状态。
/// 后端是登录状态的唯一权威。状态机：
///   idle → opening_login → waiting_for_user → collecting_cookies
///        → login_detected → bootstrapping? → verifying
///        → authenticated | failed | canceled
/// 前端只允许读取与展示，绝不允许凭"用户大概登录完了"自行断言 authenticated。
...
/// 纯函数：根据 Cookie 指标推导 WebView 登录状态。
/// credential present ≠ authenticated；只有 verify 成功才允许 authenticated。
/// cookie_present 且缺 signing key → 需要 bootstrapping。
fn qqmusic_flow_status_from_metrics(
    cookie_count: usize,
    complete: bool,
    signing_key_present: bool,
) -> &'static str {
    if !complete {
        return if cookie_count > 0 {
            "collecting_cookies"
        } else {
            "waiting_for_user"
        };
...
pub struct NeteaseServiceStatusDto {
    ...
    stage: String,
    /// P1-8: machine-readable reason when `running` is false, so the UI can
    /// say "the runtime is missing" instead of a generic "not ready".
...
async fn open_qqmusic_webview_login(
    ...
    set_qqmusic_login_flow(
        ...
        QQMusicLoginFlow::with_status(
            "opening_login",
            Some("正在打开官方登录窗口… / Opening the official sign-in window…".to_string()),
        ),
    );
...
async fn watch_qqmusic_webview_login(app: tauri::AppHandle) {
    ...
    loop {
        ...
        let has_music_key = flow.signing_key_present;

        if !complete {
            // 有部分 Cookie 但还不齐 → collecting_cookies；
            // 完全没有 → waiting_for_user（等扫码）。
            flow.status =
                qqmusic_flow_status_from_metrics(flow.cookie_count, complete, has_music_key);
            flow.message = Some(if flow.cookie_count > 0 {
                "正在收集官方会话 Cookie… / Collecting official session cookies…".to_string()
            } else {
                "等待扫码确认… / Waiting for the scan to be confirmed…".to_string()
            });
            set_qqmusic_login_flow(&state, flow);
            continue;
        }
        ...
        // 首次检测到齐备凭据 → login_detected；
        // 若缺音乐侧签名键，finalize 会 bootstrap，状态先标 bootstrapping。
        flow.status = "login_detected";
        flow.message = Some(
            "已检测到官方登录，正在准备会话… / Official sign-in detected; preparing the session…"
                .to_string(),
        );
        set_qqmusic_login_flow(&state, flow.clone());
        if !has_music_key {
            flow.status = "bootstrapping";
            flow.message = Some(
                "正在将 QQ 账号会话引导为 QQ 音乐会话… / Bootstrapping a QQ Music session…"
                    .to_string(),
            );
            set_qqmusic_login_flow(&state, flow.clone());
        }
...
async fn import_qqmusic_webview_session(
    ...
    #[cfg(windows)]
    {
        ...
        if !qqmusic_credential_is_complete(&cookie) {
            let has_any = !cookie.trim().is_empty();
            set_qqmusic_login_flow(
                &state,
                QQMusicLoginFlow::with_status(
                    if has_any {
        ...
        if extract_qqmusic_signing_key(&cookie).is_none() {
            let mut flow = QQMusicLoginFlow::metrics_from_cookie(&cookie);
            flow.status = "bootstrapping";
            flow.message = Some(
                "正在将 QQ 账号会话引导为 QQ 音乐会话… / Bootstrapping a QQ Music session…"
            ...
            flow.status = "verifying";
            flow.message =
                Some("正在确认 QQ 音乐账号… / Confirming the QQ Music account…".to_string());
            set_qqmusic_login_flow(&state, flow);
        }
...
fn resolve_netease_source_config(
    ...
    let netease_readiness = state.netease_readiness.clone();
...
// ── P1-8: NetEase sidecar readiness ─────────────────────────────────────
//
// `Command::spawn()` returning Ok only means a process was created. It does
// not mean the service bound its port, and it does not mean it answers
// requests. The previous implementation treated "a child handle exists" as
// "the service is alive", which stranded a permanently sick process: later
// requests skipped the spawn, probed once, failed, and the only recovery was
// a manual "re-detect" in Settings.
//
// Readiness is now explicit state with a single authority:
//
//   not_started ──► starting_service ──► ready
//                          │
//                          └──────────► failed
//                                        (runtime_missing | spawn_failed |
//                                         health_timeout)
//
// and a child that exists but does not answer is killed and replaced instead
// of being trusted forever.

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum NeteaseReadinessStage {
    NotStarted,
    Starting,
    Ready,
    Failed,
...
impl NeteaseReadinessStage {
    pub fn as_str(self) -> &'static str {
        match self {
            NeteaseReadinessStage::NotStarted => "not_started",
            NeteaseReadinessStage::Starting => "starting_service",
            NeteaseReadinessStage::Ready => "ready",
            NeteaseReadinessStage::Failed => "failed",
        }
    }
}

pub const NETEASE_REASON_RUNTIME_MISSING: &str = "runtime_missing";
pub const NETEASE_REASON_SPAWN_FAILED: &str = "spawn_failed";
pub const NETEASE_REASON_HEALTH_TIMEOUT: &str = "health_timeout";
pub const NETEASE_REASON_EXTERNAL_SOURCE: &str = "external_source";

#[derive(Clone, Debug)]
pub struct NeteaseReadiness {
    pub stage: NeteaseReadinessStage,
    pub reason: Option<String>,
    pub message: String,
    /// Consecutive failed transitions. Keeps restart behaviour observable and
    /// bounded instead of silently repeating forever.
    pub failed_transitions: u32,
}

impl Default for NeteaseReadiness {
    fn default() -> Self {
        NeteaseReadiness {
            stage: NeteaseReadinessStage::NotStarted,
            reason: None,
            message: "The music source has not been started yet.".to_string(),
            failed_transitions: 0,
        }
    }
}

impl NeteaseReadiness {
    pub fn mark_starting(&mut self) {
        self.stage = NeteaseReadinessStage::Starting;
        self.reason = None;
        self.message = "Music source is starting.".to_string();
    }

    pub fn mark_ready(&mut self) {
        self.stage = NeteaseReadinessStage::Ready;
        self.reason = None;
        self.failed_transitions = 0;
        self.message = "Music source is ready.".to_string();
    }

    pub fn mark_failed(&mut self, reason: &str, message: &str) {
        self.stage = NeteaseReadinessStage::Failed;
        self.reason = Some(reason.to_string());
        self.message = message.to_string();
        self.failed_transitions = self.failed_transitions.saturating_add(1);
    }
...
/// Bounded exponential backoff for the readiness probe.
///
/// Deliberately capped: an unbounded or long flat poll turns a dead sidecar
/// into a request that hangs instead of failing, which is exactly the P1-8
/// symptom. Worst case ~15s for the bundled runtime, ~31s for the npm path.
pub fn netease_probe_delay_ms(attempt: u32) -> u64 {
    const BASE_MS: u64 = 200;
    const MAX_MS: u64 = 800;
    let growth = 1u64 << attempt.min(3);
    (BASE_MS * growth).min(MAX_MS)
}

/// Probe attempts before a transition is declared failed.
pub fn netease_probe_attempts(local_runtime_found: bool) -> u32 {
...
fn netease_readiness_mark_ready(readiness: &Mutex<NeteaseReadiness>) {
    if let Ok(mut guard) = readiness.lock() {
        guard.mark_ready();
    }
}

fn netease_readiness_mark_starting(readiness: &Mutex<NeteaseReadiness>) {
    if let Ok(mut guard) = readiness.lock() {
        guard.mark_starting();
    }
}

fn netease_readiness_mark_failed(readiness: &Mutex<NeteaseReadiness>, reason: &str, message: &str) {
    if let Ok(mut guard) = readiness.lock() {
        guard.mark_failed(reason, message);
    }
}

/// Kill and reap the managed child, if any.
///
/// Used both for the restart path (a process that exists but never answered)
/// and for the cleanup path (a transition that timed out), so the next attempt
/// never inherits a process that failed to bind its port.
fn kill_managed_netease_child(child_handle: &Mutex<Option<Child>>) {
...
fn netease_ready_dto(
    base_url: &str,
    started: bool,
    node_available: bool,
    api_package_found: bool,
    message: &str,
) -> NeteaseServiceStatusDto {
    NeteaseServiceStatusDto {
        running: true,
        started,
        ...
        node_available,
        api_package_found,
        stage: NeteaseReadinessStage::Ready.as_str().to_string(),
        reason: None,
    }
}

fn netease_failed_dto(
    base_url: &str,
    node_available: bool,
    api_package_found: bool,
    stage: NeteaseReadinessStage,
    reason: &str,
    message: &str,
) -> NeteaseServiceStatusDto {
    NeteaseServiceStatusDto {
        running: false,
        started: stage == NeteaseReadinessStage::Starting,
        base_url: base_url.to_string(),
        message: message.to_string(),
        node_available,
        api_package_found,
...
async fn ensure_local_netease_api_service(
    ...
    readiness: &Mutex<NeteaseReadiness>,
) -> Result<NeteaseServiceStatusDto, String> {
    let base_url = normalize_netease_base_url(base_url);
    if !is_local_netease_base_url(&base_url) {
        return Ok(netease_failed_dto(
            &base_url,
            false,
            false,
            NeteaseReadinessStage::NotStarted,
            NETEASE_REASON_EXTERNAL_SOURCE,
            "External music source selected.",
        ));
    }
    ...
    let runtime_found = managed_api_found || api_package_found || npx_available;

    // 1. Fast path: we already completed a transition and the service still
    //    answers. Avoids re-probing from scratch on every single request.
    let known_ready = readiness
        .lock()
        .map(|guard| guard.is_ready())
        .unwrap_or(false);
    if known_ready && is_netease_api_reachable(&base_url).await {
        return Ok(netease_ready_dto(
            &base_url,
            false,
            node_available,
            runtime_found,
            "Music source is awake.",
        ));
    }

    // 2. Only one caller may run the stopped → starting → ready transition.
    //    Concurrent search, playback and settings requests wait here and share
    //    one startup instead of spawning competing node.exe jobs.
    let _startup_guard = start_lock.lock().await;

    // Another caller may have finished the transition while we waited.
    if is_netease_api_reachable(&base_url).await {
        netease_readiness_mark_ready(readiness);
        return Ok(netease_ready_dto(
            &base_url,
            false,
            node_available,
    ...
    if !node_available {
        netease_readiness_mark_failed(
            readiness,
            NETEASE_REASON_RUNTIME_MISSING,
            "The built-in NetEase runtime was not found. Please reinstall Ome Music, or set an external music source in Settings.",
        );
        return Ok(netease_failed_dto(
            &base_url,
            false,
            runtime_found,
            NeteaseReadinessStage::Failed,
            NETEASE_REASON_RUNTIME_MISSING,
            "The built-in NetEase runtime was not found. Please reinstall Ome Music, or set an external music source in Settings.",
        ));
    }
    ...
    if !runtime_found {
        netease_readiness_mark_failed(
            readiness,
            NETEASE_REASON_RUNTIME_MISSING,
            "The NetEase music source is not ready. Please reinstall Ome Music, or set an external music source in Settings.",
        );
        return Ok(netease_failed_dto(
            &base_url,
            true,
    ...
    // 3. Restart path. A child that exists but does not answer is sick, not
    //    ready — the old code skipped the spawn forever in this state, which
    //    is why the service could only be recovered by hand.
    kill_managed_netease_child(child_handle);
    netease_readiness_mark_starting(readiness);
    ...
    let child = match command.spawn() {
        Ok(child) => child,
        Err(error) => {
            let message = format!("Could not wake the music source. {error}");
            netease_readiness_mark_failed(readiness, NETEASE_REASON_SPAWN_FAILED, &message);
            return Ok(netease_failed_dto(
                &base_url,
                node_available,
                runtime_found,
                NeteaseReadinessStage::Failed,
                NETEASE_REASON_SPAWN_FAILED,
                &message,
            ));
        }
    };
    ...
    // 4. Bounded health probe with backoff. spawn() ≠ ready: we only claim
    //    readiness once the service actually answers on its port.
    let attempts = netease_probe_attempts(managed_api_found || api_package_found);
    for attempt in 0..attempts {
        ...
        tokio::time::sleep(std::time::Duration::from_millis(netease_probe_delay_ms(
            attempt,
        )))
        .await;
    }

    // 5. Give up honestly. Kill the sick child so the next attempt starts from
    //    a clean slate instead of inheriting a process that never bound.
    kill_managed_netease_child(child_handle);
    let message = if managed_api_found {
        "Music source did not become ready in time. It will be restarted on the next request."
    } else {
        "Music source is being prepared. The first launch can take a little longer — please try again in a moment."
    };
    netease_readiness_mark_failed(readiness, NETEASE_REASON_HEALTH_TIMEOUT, message);
    Ok(netease_failed_dto(
        &base_url,
        node_available,
        runtime_found,
        NeteaseReadinessStage::Starting,
        NETEASE_REASON_HEALTH_TIMEOUT,
        message,
    ))
}
...
mod tests {
    use super::{
        ...
        is_safe_remote_media_url, media_url_host, merge_bilibili_cookies, netease_probe_attempts,
        netease_probe_delay_ms, normalize_bilibili_image_url, parse_bilibili_danmaku_xml,
        request_bilibili_danmaku_xml, wbi_key_from_url, NeteaseReadiness, NeteaseReadinessStage,
        ResolvedBilibiliSourceConfig, NETEASE_REASON_HEALTH_TIMEOUT,
        NETEASE_REASON_RUNTIME_MISSING,
    };

    // ── P1-8: sidecar readiness state machine ────────────────────────────
    //
    // The spawn/health-probe sequence itself needs a real process and is not
    // unit-testable here; what IS testable — and what actually caused the bug —
    // is the readiness state that decided whether spawn was attempted at all.

    #[test]
    fn readiness_stages_map_to_stable_strings() {
        assert_eq!(NeteaseReadinessStage::NotStarted.as_str(), "not_started");
        assert_eq!(NeteaseReadinessStage::Starting.as_str(), "starting_service");
        assert_eq!(NeteaseReadinessStage::Ready.as_str(), "ready");
        assert_eq!(NeteaseReadinessStage::Failed.as_str(), "failed");
    }

    #[test]
    fn readiness_starts_unknown_and_not_ready() {
        let readiness = NeteaseReadiness::default();
        assert_eq!(readiness.stage, NeteaseReadinessStage::NotStarted);
        assert!(!readiness.is_ready());
        assert_eq!(readiness.reason, None);
        assert_eq!(readiness.failed_transitions, 0);
    }

    #[test]
    fn a_successful_transition_clears_failure_history() {
        let mut readiness = NeteaseReadiness::default();
        readiness.mark_starting();
        assert_eq!(readiness.stage, NeteaseReadinessStage::Starting);
        assert!(!readiness.is_ready());

        readiness.mark_failed(NETEASE_REASON_HEALTH_TIMEOUT, "timed out");
        readiness.mark_failed(NETEASE_REASON_RUNTIME_MISSING, "missing");
        assert_eq!(readiness.failed_transitions, 2);
        assert!(!readiness.is_ready());
    ...
    #[test]
    fn a_failed_transition_keeps_an_explicit_machine_reason() {
        let mut readiness = NeteaseReadiness::default();
        readiness.mark_failed(NETEASE_REASON_RUNTIME_MISSING, "runtime not found");

        assert_eq!(readiness.stage, NeteaseReadinessStage::Failed);
        assert_eq!(
            readiness.reason.as_deref(),
            Some(NETEASE_REASON_RUNTIME_MISSING)
        );
    }

    #[test]
    fn failing_repeatedly_stays_bounded_and_counted() {
        let mut readiness = NeteaseReadiness::default();
        for _ in 0..10 {
            readiness.mark_failed(NETEASE_REASON_HEALTH_TIMEOUT, "timed out");
        }
        // saturating_add: the counter never wraps back to zero, which would
        // silently reset any restart policy built on top of it.
        assert_eq!(readiness.failed_transitions, 10);
    }

    #[test]
    fn health_probe_backoff_is_bounded_and_never_zero() {
        let mut previous = 0;
        for attempt in 0..40 {
            let delay = netease_probe_delay_ms(attempt);
            assert!(delay >= 200, "delay must stay non-trivial: {delay}");
            assert!(delay <= 800, "delay must stay bounded: {delay}");
            assert!(delay >= previous, "backoff must not shrink: {delay}");
            previous = delay;
        }
    }

    #[test]
    fn total_probe_window_stays_bounded() {
        // The regression was a request that hung instead of failing, so the
        // worst-case wait is part of the contract, not an implementation detail.
        for local_runtime in [true, false] {
            let attempts = netease_probe_attempts(local_runtime);
            let total: u64 = (0..attempts).map(netease_probe_delay_ms).sum();
            assert!(total <= 35_000, "probe window too long: {total}ms");
            assert!(
                total >= 1_000,
                "probe window too short to be useful: {total}ms"
            );
        }
    }
    ...
    #[test]
    fn qqmusic_flow_status_never_treats_partial_cookies_as_authenticated() {
        assert_eq!(
            super::qqmusic_flow_status_from_metrics(0, false, false),
            "waiting_for_user"
        );
        assert_eq!(
            super::qqmusic_flow_status_from_metrics(2, false, false),
            "collecting_cookies"
        );
        // QQ 账号 Cookie 齐了但缺音乐侧签名键 → 必须 bootstrap，不能 authenticated。
        assert_eq!(
            super::qqmusic_flow_status_from_metrics(4, true, false),
            "bootstrapping"
        );
        assert_eq!(
            super::qqmusic_flow_status_from_metrics(5, true, true),
            "login_detected"
        );
        for status in [
            super::qqmusic_flow_status_from_metrics(0, false, false),
            ...
            assert_ne!(status, "authenticated");
        }
    }

    #[test]
    fn qqmusic_login_flow_metrics_never_leak_cookie_values() {
        let flow = super::QQMusicLoginFlow::metrics_from_cookie(
            "uin=o0012345; qm_keyst=do-not-print==; p_skey=private",
        );
        assert_eq!(flow.cookie_count, 3);
        assert!(flow.uin_present);
        assert!(flow.signing_key_present);
        assert_eq!(flow.cookie_names, vec!["uin", "qm_keyst", "p_skey"]);
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            app.manage(AppState {
                ...
                netease_readiness: Arc::new(Mutex::new(NeteaseReadiness::default())),
            });
            Ok(())
        })
